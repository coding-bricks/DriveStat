...
public class DriveStatMainController {
	...
	ChangeListener<Pattern> patternsChangeListener;
	ChangeListener<BeamProductionChain> chainsChangeListener;
	...
	private void handleRefresh() {
		...
		if (!listOfPatternshasChanged) {
			...
			dm.setIconAndText(null,
					"Update of Patterns/Chains currently available. Patterns and Chains will be shortly updated.");
	...
	private boolean listOfPatternsUpdater() {
		...
		/*
		 * patternsAndChainsMap =
		 * retrievePatternsAndChains.getPatternsAndChains();
		 *
		 * // Check if patterns in combobox are not anymore existing for (int i
		 * = 0; i < patterns_cb.getItems().size(); i++) {
		 *
		 ...
		 * break; }
		 *
		 * if (counter == patternsAndChainsMap.size() - 1) { // if element is
		 * not found then listOfPatternshasChanged = true; // break; }
		 *
		 * counter++; }
		 *
		 * }
		 *
		 * // Check if new patterns are present for (final Pattern patternKey :
		 * patternsAndChainsMap.keySet()) {
		 *
		 ...
		 * patterns_cb.getItems().add(patternKey); listOfPatternshasChanged =
		 * true; } } }
		 *
		 * if (listOfPatternshasChanged) {
		 *
		 ...
		 *
		 * System.out.
		 * println("No changes have been found in the list of Patterns!!!!"); }
		 *
